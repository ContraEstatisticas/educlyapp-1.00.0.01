...
export const FindPromptError = ({
  ...
  const correctErrorIndex = useMemo(
    () => promptParts.findIndex((part) => part.hasError),
    [promptParts],
  );
  ...
  return (
    <div className="w-full space-y-6">
      <div className="flex items-start gap-4">
        ...
        <div>
          {title ? <h3 className="text-lg font-bold text-foreground mb-1">{title}</h3> : null}
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className={cn("font-semibold", isCorrect ? "text-green-800" : "text-red-800")}>
                {isCorrect
                  ? tUi(t, i18n.language, "lesson.findError.correct")
                  : tUi(t, i18n.language, "lesson.findError.incorrect")}
              </p>
              ...
              {!isCorrect && attempts >= 2 ? (
                <div className="mt-3 flex items-start gap-2 text-orange-700 bg-orange-50 rounded-xl p-3 border border-orange-200">
                  ...
                  <p className="text-sm">{tUi(t, i18n.language, "lesson.findError.hint")}</p>
                </div>
              ) : null}
            </div>
          </div>
