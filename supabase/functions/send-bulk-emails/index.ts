...
serve(async (req) => {
  ...
  try {
    ...
    if (targetEmails && Array.isArray(targetEmails) && targetEmails.length > 0) {
      ...
      users = users.filter((u: any) => u.email && targetEmailsLower.includes(u.email.toLowerCase()));
    }

    const { data: profiles } = await supabase.from('profiles').select('id, full_name, preferred_language');
    ...
    for (const user of users) {
      ...
      const profile = profileMap.get((user as any).id) as any;
      const userName = profile?.full_name || (user as any).user_metadata?.full_name || email.split('@')[0];
      const language = (profile?.preferred_language || (user as any).user_metadata?.preferred_language || 'en').toLowerCase().split('-')[0];
      ...
      try {
        ...
        results.push({ email, success: false, error: error instanceof Error ? error.message : "Unknown" });
      }

      await delay(5000);
    }

    return new Response(JSON.stringify({ totalUsers: users.length, emailsSent: successCount, emailsFailed: users.length - successCount, results }), {
