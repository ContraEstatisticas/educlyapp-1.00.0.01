...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (createUserError || !createdUserData.user) {
      console.error("[pending-signup] createUser failed:", createUserError);
    ...
    if (profileError) {
      console.warn("[pending-signup] profile language update failed:", profileError);
    }

    return jsonResponse({ user_id: userId });
  } catch (error) {
    console.error("[pending-signup] unexpected error:", error);
    return jsonResponse(
