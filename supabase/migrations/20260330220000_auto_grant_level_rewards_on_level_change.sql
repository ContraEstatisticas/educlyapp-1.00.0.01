CREATE OR REPLACE FUNCTION public.apply_level_rewards_internal(p_user_id UUID, p_current_level INTEGER)
...
DECLARE
  ...
  v_existing_level5_row user_level_rewards%ROWTYPE;
BEGIN
  SELECT au.email
  INTO v_user_email
  FROM auth.users au
  ...
  IF p_current_level >= 3 THEN
    ...
    ON CONFLICT ON CONSTRAINT user_level_rewards_user_level_unique DO UPDATE SET
      metadata = EXCLUDED.metadata,
      updated_at = NOW()
    WHERE (COALESCE(public.user_level_rewards.metadata, '{}'::jsonb) - 'popup_seen_at') IS DISTINCT FROM EXCLUDED.metadata
  ...
  IF p_current_level >= 5 THEN
    ...
    IF NOT FOUND THEN
      IF v_has_ai_hub THEN
        ...
        ON CONFLICT ON CONSTRAINT user_level_rewards_user_level_unique DO NOTHING
        RETURNING * INTO v_inserted_row;
...
BEGIN
  ...
  RETURN QUERY
  SELECT *
  FROM public.apply_level_rewards_internal(p_user_id, p_current_level);
END;
$$;

GRANT EXECUTE ON FUNCTION public.apply_level_rewards(UUID, INTEGER) TO authenticated;

CREATE OR REPLACE FUNCTION public.handle_user_level_rewards_sync()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
BEGIN
  PERFORM public.apply_level_rewards_internal(NEW.user_id, NEW.current_level);
  RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS sync_user_level_rewards ON public.user_levels;

CREATE TRIGGER sync_user_level_rewards
AFTER INSERT OR UPDATE OF current_level, total_xp_earned
ON public.user_levels
FOR EACH ROW
EXECUTE FUNCTION public.handle_user_level_rewards_sync();
...
BEGIN
  ...
  LOOP
    PERFORM public.apply_level_rewards_internal(v_level_row.user_id, v_level_row.current_level);
  END LOOP;
END $$;
