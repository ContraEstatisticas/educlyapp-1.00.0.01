...
BEGIN
  ...
  IF p_current_level >= 3 THEN
    ...
    ON CONFLICT (user_id, source_level) DO UPDATE SET
...
WITH eligible_level5 AS (
  SELECT
    ul.user_id,
    LOWER(COALESCE(au.email, '')) AS user_email,
    (
      EXISTS (
        SELECT 1
        FROM public.user_product_access upa
...
SELECT
  ...
  NOW() + INTERVAL '1 day',
  NULL
FROM eligible_level5
WHERE NOT eligible_level5.has_ai_hub
ON CONFLICT (user_id, product_id) DO UPDATE SET
  product_type = EXCLUDED.product_type,
  is_active = true,
  granted_at = NOW(),
...
SELECT
  eligible_level5.user_id,
  CASE
    WHEN eligible_level5.has_ai_hub THEN 'prompt_guide_pdf'
    ELSE 'ai_hub_day_pass'
  END,
  5,
  CASE
    WHEN eligible_level5.has_ai_hub THEN jsonb_build_object(
      'download_url', '/rewards/educly-ai-prompts-guide.pdf',
      'status', 'available'
    )
...
INSERT INTO public.user_level_rewards (user_id, reward_key, source_level, metadata)
SELECT
  eligible_level7.user_id,
  'ai_hub_bonus_limits',
  7,
  CASE
    WHEN eligible_level7.has_ai_hub THEN jsonb_build_object(
      'status', 'active',
      'cta_path', '/assistentes',
      'extra_messages', 20,
      'extra_images', 5,
