-- Allow specialized trail certificates to coexist with challenge certificates
-- and validate completion against server-side module counts.

ALTER TABLE public.user_certificates
  DROP CONSTRAINT IF EXISTS user_certificates_user_id_tool_slug_key;

CREATE UNIQUE INDEX IF NOT EXISTS user_certificates_user_tool_type_no_challenge_uidx
  ON public.user_certificates (user_id, tool_slug, certificate_type)
  WHERE challenge_id IS NULL;

CREATE UNIQUE INDEX IF NOT EXISTS user_certificates_user_tool_type_challenge_uidx
  ON public.user_certificates (user_id, tool_slug, certificate_type, challenge_id)
